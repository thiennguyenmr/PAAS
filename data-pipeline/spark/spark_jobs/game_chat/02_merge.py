...
def main():
    spark = get_spark_session("03_Merge_GameChat_CSV_with_Existing")

    # Paths
    csv_source_path = "s3a://raw/csv_chat_messages"
    db_source_path = "s3a://raw/db_chat_messages"
    merged_output_path = "s3a://raw/merged_chat_messages"

    print("="*60)
    print("MERGING CSV CHAT DATA WITH EXISTING DATABASE DATA")
    print("="*60)
    ...
    if df_csv is None:
        ...
        # Deduplication strategy: Keep the latest record per message_id based on timestamp
        # If timestamps are equal, prefer database source
        window_spec = Window.partitionBy("message_id").orderBy(
            F.desc("timestamp"),
    ...
    # Check message types
    valid_types = ['public', 'private', 'system', 'game_announcement', 'support']
    invalid_types = df_merged.filter(~F.col("message_type").isin(valid_types)).count()

    if invalid_types > 0:
        print(f"   - WARNING: Found {invalid_types} records with invalid message types!")

    # Check for empty messages
    empty_msgs = df_merged.filter(
        (F.trim(F.col("message_content")) == "") |
        (F.length(F.col("message_content")) == 0)
    ).count()

    if empty_msgs > 0:
        print(f"   - WARNING: Found {empty_msgs} empty messages!")
    ...
    df_merged.select(
        ...
        "message_type", "channel", "message_content", "data_source"
    ).show(10, truncate=50)

    # Statistics by message type
    print("\n6. Message type distribution:")
    df_merged.groupBy("message_type", "data_source") \
        .count() \
        .orderBy("message_type", "data_source") \
        .show()

    # Write merged data
    print(f"\n7. Writing merged data to {merged_output_path}")
    try:
