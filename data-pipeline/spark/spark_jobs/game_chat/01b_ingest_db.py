...
def main():
    spark = get_spark_session("01_Ingest_GameChat_Postgres_to_MinIO_Raw")

    print("DEBUG: Spark Version:", spark.version)

    print("Reading chat messages from Postgres...")
    try:
        connector = PostgresConnector()
        df = connector.read_table(spark, "chat_messages")

        # Count source records
        source_count = df.count()

        print("DEBUG: Chat Messages DataFrame Schema:")
        df.printSchema()
        print("DEBUG: Chat Messages Data Sample:")
    ...
    output_path = "s3a://raw/db_chat_messages"
    ...
    try:
        ...
        validate_record_count(spark, source_count, output_path, "Chat PostgreSQL Ingestion")
