"""
HiLo Game Round Data Pipeline DAG
Orchestrates the complete data pipeline from ingestion to graph loading
"""
...
# DAG definition
dag = DAG(
    'hilo_game_round_pipeline',
    default_args=default_args,
    description='Complete HiLo game round data pipeline: Ingest → Merge → Quality Check → Clean → Transform → Vectorize → Graph',
    #schedule='0 2 * * *',  # Run daily at 2 AM
    catchup=False,
    tags=['hilo', 'game', 'data-pipeline', 'spark'],
)

# Base path for Spark jobs
SPARK_BASE_DIR = "/opt/spark/work-dir"
SPARK_JOBS_DIR = f"{SPARK_BASE_DIR}/spark_jobs/game_round"
...
ingest_csv = SparkSubmitOperator(
    ...
    name='hilo-gameround-ingest-csv',
...
graph_load = SparkSubmitOperator(
    ...
    name='hilo-gameround-graph-load',
    conn_id='spark_default',
    verbose=True,
    conf=spark_conf,
    dag=dag,
